...
-- Explicit readfile per module — no path variable (Xeno safe)
-- ══════════════════════════════════════════════════════════════
local function execModule(source, name)
...
-- ══════════════════════════════════════════════════════════════
-- LOAD ALL 7 MODULES  (strict dependency order)
-- Each readfile call is hardcoded — no path concatenation (Xeno safe)
-- ══════════════════════════════════════════════════════════════
-- 1. Profile first — provides ctx.P, ctx.ka/kb/aim/esp/fpsState,
--    ctx.saveProfile, ctx.collectAndSave
execModule(readfile("Module7_Profile.lua"), "Module7_Profile.lua")

-- 2. GUI — provides ctx.Window, ctx.PURPLE_* colors
execModule(readfile("Module1_GUI.lua"), "Module1_GUI.lua")

-- 3. Kill Aura — provides ctx.entitylib, ctx.bedwars*
execModule(readfile("Module2_KillAura.lua"), "Module2_KillAura.lua")

-- 4. KB Reducer — provides ctx.CombatTab
execModule(readfile("Module3_KBReducer.lua"), "Module3_KBReducer.lua")

-- 5. Aim Assist — appends to ctx.CombatTab, provides ctx.doAimAssist
execModule(readfile("Module4_AimAssist.lua"), "Module4_AimAssist.lua")
...
execModule(readfile("Module5_FPSBoost.lua"), "Module5_FPSBoost.lua")

-- 7. Credits + Changelog
execModule(readfile("Module6_Credits.lua"), "Module6_Credits.lua")

-- 8. Kit ESP -- adds section to ESPTab (needs ctx.ESPTab from Module5)
execModule(readfile("Module8_KitESP.lua"), "Module8_KitESP.lua")

-- 9. Animations tab -- needs ctx.Window from Module1
execModule(readfile("Module9_Animations.lua"), "Module9_Animations.lua")
